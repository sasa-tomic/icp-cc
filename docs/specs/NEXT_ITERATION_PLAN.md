...
- **Status:** PLANNING deliverable. Extends `NEXT_PHASE_PLAN.md` (COMPLETE),
...
- **New debt found:** TD-8 (clipboard slop — a real user-facing bug), and the
  residual 9 status-color literals (TD-9, the TD-5 finish-up).
...
#### TD-8 — Fix the `canister_call_builder` clipboard slop (user-facing lie)
...
#### TD-9 — Sweep the residual 9 status-color literals to tokens (TD-5 finish-up)
- **Problem:** 9 `Colors.(green|red|orange)` literals remain, all encoding status
  semantics that already have tokens (`AppDesignSystem.{success,warning,error}Color`).
- **Grounding:** `bookmarks_screen.dart:1051,1109,1206,1619` (orange=query tint),
  `script_details_dialog.dart:534,665` (orange status badge),
  `script_card.dart:479,484,495` (red=delete/error).
- **Change:** replace each with the symbolic token (`.withValues(alpha:)` for the
  translucent variants). Value-preserving.
- **Risk:** LOW. **Confidence 9/10.**
- **Commit:** `refactor(theme): TD-9 sweep residual status-color literals to tokens`

#### TD-13 — Make the vault code comments honest (interim, pending A-4 decision)
...
#### UX-11 — Poll dapp auto-loads on open (not "Polls (0)" + manual Refresh)
- **Problem:** the bundle's `init` returns `effects: []`, so the dapp opens to
  "Polls (0)" — contradicting the plan's *"sees a real canister respond … One
  click, no setup"*. The `refreshEffects` helper already exists.
- **Grounding:** `lib/examples/06_icp_poll.js:18-32` (`init` → `effects: []`);
  `:197-202` (`refreshEffects` builds the whoami+listPolls batch).
- **Change:** have `init` return `refreshEffects()`'s output (or emit a
  self-`{type:"refresh"}`), so polls/principal load on mount. ~2-line bundle change.
- **Risk:** LOW. **Confidence 9/10.**
- **Commit:** `fix(dapps): UX-11 poll bundle auto-loads on open`

#### UX-13 — Fix the DISPLAY double-prefix in `run-with-mock-keyring.sh`
...
#### UX-12 — Dapps Connection panel honesty
...
#### TQ-4 — Fold the marketplace test `MaterialApp` harness (mirror TQ-3)
...
```
  Wave 1 (parallel, independent):
    TD-8  TD-9  TD-13  UX-11  UX-13  UX-12  TQ-4
  ...
  Flagged (NOT auto-executed):
    A-4 (vault zero-knowledge migration) ─► human decision
...
just test-feature marketplace    # TD-9, TQ-4
just test-feature dapps          # UX-11, UX-12
cd apps/autorun_flutter && flutter analyze
just test                        # Rust + Flutter full suite
rg "Colors\.(green|red|orange)\b" apps/autorun_flutter/lib | wc -l   # TD-9: ~0 (tokens only)
