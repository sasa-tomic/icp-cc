...
## Project Purpose
ICP Autorun is a Flutter application for running Lua scripts with Internet Computer (ICP) integration. It includes a Rust core for Lua execution and a Cloudflare Workers API backend.

## Tech Stack
- **Frontend**: Flutter (Dart)
- **Core Engine**: Rust (mlua crate for Lua execution) 
- **Backend**: Cloudflare Workers (TypeScript/JavaScript)
...
- `just test`: Run complete test suite (Rust + Flutter + Cloudflare)
- `just _rust-tests`: Rust linting, formatting, and tests
- `just _flutter-tests`: Flutter analysis and tests
- `just cloudflare-dev`: Local development server
