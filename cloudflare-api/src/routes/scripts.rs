...
pub async fn handle_scripts_request(req: Request, env: &AppEnv) -> Result<Response> {
    ...
    match req.method() {
        Method::Get => {
            let query_params = match req.query::<ScriptsQueryParams>() {
                Ok(params) => params,
                Err(_) => ScriptsQueryParams::default(),
            };

            let limit = query_params.limit.unwrap_or(20);
            let offset = query_params.offset.unwrap_or(0);
            let is_public = query_params.is_public.unwrap_or(true);
...
pub async fn handle_create_script_request(mut req: Request, env: &AppEnv) -> Result<Response> {
    let db = DatabaseService::new(env);
...
pub async fn handle_get_script_request(req: Request, env: &AppEnv, script_id: &str) -> Result<Response> {
    ...
    let include_private = query_params.include_private.unwrap_or(false);
...
pub async fn handle_update_script_request(mut req: Request, env: &AppEnv, script_id: &str) -> Result<Response> {
    if script_id.is_empty() {
        return Ok(JsonResponse::error("Script ID is required", 400));
    }
        ...
        .await
    {
        Ok(Some(existing_script)) => {
            let existing_principal = existing_script.get("author_principal")
                .and_then(|v| v.as_str()).ok_or_else(|| {
                    worker::Error::JsError("No author principal found".to_string())
                })?;
            let existing_public_key = existing_script.get("author_public_key")
                .and_then(|v| v.as_str()).ok_or_else(|| {
                    worker::Error::JsError("No public key found".to_string())
                })?;
...
pub async fn handle_delete_script_request(mut req: Request, env: &AppEnv, script_id: &str) -> Result<Response> {
    if script_id.is_empty() {
        return Ok(JsonResponse::error("Script ID is required", 400));
    }
...
pub async fn handle_publish_script_request(req: Request, env: &AppEnv, script_id: &str) -> Result<Response> {
...
pub async fn handle_scripts_by_category_request(req: Request, env: &AppEnv, category: &str) -> Result<Response> {
    if req.method() != Method::Get {
        return Ok(JsonResponse::error("Method not allowed", 405));
    }

    let db = DatabaseService::new(env);
    let query_params = match req.query::<CategoryQueryParams>() {
        Ok(params) => params,
        Err(_) => CategoryQueryParams::default(),
    };

    let limit = query_params.limit.unwrap_or(20);
    let offset = query_params.offset.unwrap_or(0);
    let sort_by = query_params.sort_by.unwrap_or("rating".to_string());
    let sort_order = query_params.sort_order.unwrap_or("desc".to_string());
...
pub async fn handle_scripts_count_request(_req: Request, env: &AppEnv) -> Result<Response> {
    let db = DatabaseService::new(env);
    let database = db.get_database();
