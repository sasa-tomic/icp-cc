...
/// CLI entry point. Parses --count / --endpoint / --clean / --purge, then
/// uploads (or purges).
Future<void> main(List<String> args) async {
  ...
  print('==> seed_marketplace: endpoint=$endpoint '
      'mode=${opts.purge ? "purge" : "seed"} count=${opts.count} '
      'clean=${opts.clean}');
  ...
  // --purge: discover ALL bulk-seed scripts by this keypair and DELETE them.
  // Use this to reset the backend to a clean state before seeding. Each
  // delete is signed (real Ed25519 over the canonical delete payload) so the
  // backend's auth + ownership checks pass.
  if (opts.purge) {
    final existing = await _discoverExistingBulkSeedsWithIds(
        client, searchUri, publicKeyB64);
    print('   purge: ${existing.length} bulk_seed scripts to delete.');
    var deleted = 0;
...
Future<bool> _uploadOne({
  ...
  final slug = 'bulk-seed-script-$index';
  final description = 'Bulk seed script #$index for e2e pagination testing. '
      'Generated by tool/seed_marketplace.dart.';
  final category = 'utility';
  final bundle = 'print("Bulk seed $index");';
  final version = '1.0.0';
  // tags must be sorted in the payload (matches backend's
  // build_upload_payload: `sorted_tags.sort()`).
  final tags = <String>['bulk_seed', 'pagination', 'seed']
    ..sort();
  ...
  try {
    ...
    stderr.writeln('   ✗ [$index] HTTP ${response.statusCode}: '
        '${responseBody.length > 300 ? '${responseBody.substring(0, 300)}…' : responseBody}');
    return false;
  } catch (e) {
    stderr.writeln('   ✗ [$index] exception: $e');
    return false;
  }
}
...
/// the purge path (needs the id to DELETE by).
...
_Options _parseArgs(List<String> args) {
  ...
  return _Options(
      count: count, endpoint: endpoint, clean: clean, purge: purge, help: help);
}
...
void _printUsage() {
  ...
  print('  --purge            Discover and DELETE all bulk_seed scripts by');
  print('                     this seeder\'s keypair, then exit. Use to reset');
  print('                     the backend before a fresh seed.');
